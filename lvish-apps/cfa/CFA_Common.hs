...
k_param =
  case Prelude.lookup "KPARAM" theEnv of
    Just n ->
      trace ("Setting K for K-CFA to: " ++ n) $
        read n
    Nothing -> 2
...
fvExample =
  let_ "id" (lam ["x", "k"] (call (ref "k") [ref "x"])) $
    call
      (ref "id")
      [ lam ["z"] (call (ref "escape") [ref "z"])
      , lam
          ["a"]
          ( call
              (ref "id")
              [ lam ["y"] (call (ref "escape") [ref "y"])
...
scaleExample n ex =
  let f s = s ++ show n
   in let_ "const" (lam [f "x", f "y", f "k"] (call (ref (f "k")) [ref (f "x")])) $
        call
          (ref "const")
          [ lam [f "ignore1_"] (scaleExample (n - 1) ex)
          , lam [f "ignore2_"] ex
