...
nbrs AdjacencyGraph {vertOffets, allEdges} nid =
  let ind = vertOffets U.! (fromIntegral nid)
      ...
      suff = U.drop (fromIntegral ind) allEdges
   in if fromIntegral nid == U.length vertOffets - 1
        then suff
...
parseAdjacencyGraph chunks bs =
  case B.splitAt (B.length tag) bs of
    (fst, rst)
      | fst /= tag -> error $ "readAdjacencyGraph: First word in file was not " ++ B.unpack tag
      | otherwise -> do
...
readNumFile
  :: forall nty
   . (U.Unbox nty, Integral nty, Eq nty, Show nty, Read nty)
  => FilePath
  -> IO [U.Vector nty]
readNumFile path = do
...
parReadNats
  :: forall nty d s
   . (U.Unbox nty, Num nty, Eq nty)
  => Int
  -> S.ByteString
  -> Par d s [PartialNums nty]
...
data PartialNums n
  = Compound !(Maybe (RightFrag n)) ![U.Vector n] !(Maybe (LeftFrag n))
  | Single !(MiddleFrag n)
...
sewEnds origls = loop Nothing origls
 where
  ...
  sew mleft rf ls =
    case (mleft, rf) of
      (Just (LeftFrag n), Just (RightFrag nd m)) ->
        let num = shiftCombine n m nd
         in U.singleton num : ls
...
readNatsPartial bs
      ...
      if digit hd
        then
          ( let first = U.head $ head vs -- The first (possibly partial) number parsed.
                ...
                rfrag = Just (RightFrag (fromJust $ S.findIndex (not . digit) bs) first)
             in if total == 0
                  then case lfrg of
 ...
 where
  ...
  loop !lmt !ind !acc !vec !vecacc !nxt !rst
    ...
    | digit nxt =
        let acc' = (10 * acc + (fromIntegral nxt - 48))
         in if lmt == 1
  ...
  closeOff vec vecacc ind frag =
    do
      vec' <- U.unsafeFreeze (M.take ind vec)
...
unitTests =
  [ TestCase $
      assertEqual "t1" (Compound (Just (RightFrag 3 (123 :: Word))) [U.fromList []] Nothing)
        =<< readNatsPartial (S.take 4 "123 4")
  , TestCase $
      assertEqual "t1" (Compound (Just (RightFrag 3 (123 :: Word))) [U.fromList []] (Just (LeftFrag 4)))
        =<< readNatsPartial (S.take 5 "123 4")
  , TestCase $
      assertEqual "t3" (Single (MiddleFrag 3 (123 :: Word)))
        =<< readNatsPartial (S.take 3 "123")
  , TestCase $
      assertEqual "t4" (Single (MiddleFrag 2 (12 :: Word)))
        =<< readNatsPartial (S.take 2 "123")
  , TestCase $
      assertEqual "t5" (Compound Nothing [] (Just (LeftFrag (12 :: Word32))))
        =<< readNatsPartial (S.take 3 " 123")
  , TestCase $
      assertEqual
        "t6"
...
t2 = do
  ...
  t2_ <- getCurrentTime
  putStrLn $
    "Time parsing/reading "
      ++ show (diffUTCTime t1_ t0_)
      ++ " and coalescing "
      ++ show (diffUTCTime t2_ t1_)

-- This one is fast... but WHY?  It should be the same as the hacked 1-chunk parallel versions.
t3 :: IO [PartialNums Word]
t3 = do
  bs <- unsafeMMapFile "../../pbbs/breadthFirstSearch/graphData/data/3Dgrid_J_10000000"
...
t3B = do
  putStrLn "Sequential version + readFile"
  t0_ <- getCurrentTime
...
t4 = do
  putStrLn $ "Using parReadNats + readFile"
  t0_ <- getCurrentTime
  bs <- S.readFile "../../pbbs/breadthFirstSearch/graphData/data/3Dgrid_J_10000000"
  t1_ <- getCurrentTime
  putStrLn $ "Time to read file sequentially: " ++ show (diffUTCTime t1_ t0_)
