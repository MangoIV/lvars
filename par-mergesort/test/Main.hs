...
unitTests =
  testGroup
    "Hand-crafted tests and regression tests"
    [ testCase "Sorting an already sorted vector" $
        testAllVariants 10 10 $
          SV.fromList [0 .. 99 :: Int32]
    , testCase "Sorting reversed sorted vector" $
        testAllVariants 10 10 $
          SV.fromList [99 .. 0 :: Int32]
    ...
    , testCase "Public sort interface / Int32" $
        assertBool "" $
          checkSorted 11 $
            MS.sort (SV.fromList $ 99 : [1 .. 10 :: Int32])
...
properties = testProperty "QuickCheck tests" $ do
  ...
  let ret =
        sortPV
          ...
          vec
  return $
    flip counterexample (checkSorted vecSize ret) $
      unlines $
        [ "Size: " ++ show vecSize
...
checkSorted
  :: (Show elt, SVM.Storable elt, Ord elt)
  => Int
  -> SV.Vector elt
  -> Bool
checkSorted len v
