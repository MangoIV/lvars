...
mergeSort_int32
  ...
  => Int
  -- ^ Sequential sort threshold
  -> Int
  -- ^ Sequential merge threshold
  -> SSort
  -> SMerge
...
pMergeTo2 threshold seqMerge = do
  ...
  if l1 == 0 && l2 == 0
    then return ()
      ...
      if l1 < threshold || l2 < threshold || l1 <= 1 || l2 <= 1
        then do
          seqMerge
        else do
  ...
  if (lIndex == 0) && (rIndex == 0)
    then do
      ...
      if lIndex == 0
        then do
        ...
        else --           else trace ((show rIndex) ++ " " ++ (show lIndex) ++ "sp") $ split 0 0 rLow rIndex
          ...
          if rIndex == 0
            ...
            else --          else trace ((show rIndex) ++ " " ++ (show lIndex) ++ "spl") $ split lLow lIndex 0 0
            do
              rightSub1 <- MV.read vr (rIndex - 1)
              leftSub1 <- MV.read vl (lIndex - 1)
...
morphToVec21 sp (STTup2 (VFlp vec1) vec2) =
  let l1 = MV.slice 0 sp vec1
      r1 = MV.slice sp (MV.length vec1 - sp) vec1
   in STTup2 (STTup2 (VFlp l1) (VFlp r1)) vec2
        ...
        let di' = di + 1
         in if lx < rx
              then do
                ...
                let li' = li + 1
                if li' == lenL
                  then -- copy the rest of right into dest
                    copyRemainingRight ri rx di'
                  else when (di' < len) $ do
                    lx' <- MV.read left li'
                    loop li' lx' ri rx di'
                ...
                let ri' = ri + 1
                if ri' == lenR
                  then -- copy the rest of left into dest
                    copyRemainingLeft li lx di'
                  else when (di' < len) $ do
                    rx' <- MV.read right ri'
                    loop li lx ri' rx' di'
...
mkCilkSeqSort
  :: (Ord eL, ParThreadSafe p, PC.ParMonad p)
  => SeqSort eL
...
mkCilkSeqMerge
  :: (Ord e1, ParThreadSafe p, PC.ParMonad p)
  => SeqMerge e1
  -> ParVec21T s1 e1 p e s ()
...
sMergeTo2K
  ...
  -> Int
  -> ParST
      (STTup2 (STTup2 (SVectorFlp a) (SVectorFlp a)) (SVectorFlp a) s1)
      p
      e
      s
      ()
...
_morphToVec12 sp (STTup2 (VFlp vec1) (VFlp vec2)) =
  let l2 = MV.slice 0 sp vec2
      r2 = MV.slice sp (MV.length vec2 - sp) vec2
   in STTup2 (VFlp vec1) (STTup2 (VFlp l2) (VFlp r2))
