...
prop_traverse ls = mkSimpleIdentityProp fn (zip ls ls)
 where
  fn mp =
    IM.traverseMap (\_ x -> return (x + 1)) mp
...
v8c = fmap (L.sort . F.toList) $
  runParNonDet $
    ioButNoBump $ do
      ...
      m2 <- IM.newEmptyMap
      let cb k v = do
            logDbgLn 1 $ " [v8c]  Inside callback for Map.. key=" ++ show k
...
v8d = do
  ...
  let cb k v = do
        logDbgLn 1 $ " [v8d]  Inside callback for traverse.. key=" ++ show k
        return (v + 100)
...
case_v9a :: Assertion
case_v9a =
  assertEqual "make sure there's sufficient parallelism" ()
    =<< ( assertNoTimeOut 1.0 $
            runParNonDet $
              ioButNoBump $ do
                mp <- IM.newEmptyMap
