...
v9f = runParNonDet $ isND $ do
  ...
  fork
    ( do
        logDbgLn 1 " [v9f] Beginning putter loop.."
        forM_ [0 .. size - 1] $ \ix ->
  ...
  let loop !acc ix
        ...
        | otherwise = do
            ...
            when (ix `mod` 1000 == 0) $
              ...
              logDbgLn 2 $
                "   [v9f] get completed at: " ++ show ix ++ " -> " ++ show v
            loop (acc + v) (ix + 1)
  loop 0 0
...
case_v9f2_seq_fillIvarArray =
  timeOutWarning 3.0 $ -- FIXME: KNOWN PROBLEM. Livelocks here!
    assertEqual "Array of ivars, compare effficiency:" out9e
      =<< runParNonDet
        ( isND $ do
            let size = in9e
                news = V.replicate size IV.new
            ...
            let loop !acc ix
                  ...
                  | otherwise = do
                      ...
                      when (ix `mod` 1000 == 0) $
                        logDbgLn 2 $
                          "   [v9f2] get completed at: " ++ show ix ++ " -> " ++ show v
                      loop (acc + v) (ix + 1)
            fut <- spawn (loop 0 0)
            putters
