...
import Data.LVar.LayeredSatMap
  ( LSMContents (..)
  , LayeredSatMap (..)
  )
...
insert !elm !fs@(FiltSet (WrapLVar lv)) =
  let insert' e (x : y : right, left) = ((e : right, filter notSat [x, y] ++ left), Just e)
...
fromFiltSet
  :: (PartialJoinSemiLattice v, Ord v, Ord k)
  => FiltSet Frzn (LayeredSatMap k) v
  -> S.Set (M.Map k v)
