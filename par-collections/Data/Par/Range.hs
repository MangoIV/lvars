...
data Range = InclusiveRange
  { startInd :: {-# UNPACK #-} !Int
  -- ^ Start, inclusive
  , endInd :: {-# UNPACK #-} !Int
  -- ^ End, inclusive
  , seqThresh :: {-# UNPACK #-} !Int
  -- ^ For ranges less than or equal to this, switch to sequential execution.
  }
...
pmapReduce
  ...
  => Range
  -- ^ iteration range over which to calculate
  -> (Int -> p e s a)
  -- ^ compute one result
  -> (a -> a -> p e s a)
  -- ^ combine two results
...
{-# INLINE mkMapReduce #-}
mkMapReduce
  :: (ParFuture m, HasPut e, HasGet e, FutContents m t)
