...
newMap mp =
  fmap (IMap . WrapLVar) $ WrapPar $ newLV $ do
    ...
    traverseWithKey_
      ( \k v -> do
          CM.Added _ <- CM.putIfAbsent cm k (return v)
          return ()
      )
      mp
    return cm
...
insert
  :: (Ord k, Hashable k, Eq v, HasPut e)
  => k
  -> v
  -> IMap k s v
  -> Par e s ()
...
modify
  :: forall f a b e s key
   . (Ord key, Hashable key, Show key, Ord a, HasPut e)
  => IMap key s (f s a)
  -> key
  -- ^ The key to lookup.
  -> (Par e s (f s a))
...
traverseMap
  :: (Ord k, Hashable k, Eq b, HasPut e, NoBump e)
  => (k -> a -> Par e s b)
  -> IMap k s a
  -> Par e s (IMap k s b)
...
copy
  :: (Ord k, Hashable k, Eq v, HasPut e, NoBump e)
  => IMap k s v
  -> Par e s (IMap k s v)
...
unionHP
  :: (Ord k, Hashable k, Eq a, HasPut e, NoBump e)
  => Maybe HandlerPool
  -> IMap k s a
  -> IMap k s a
  -> Par e s (IMap k s a)
