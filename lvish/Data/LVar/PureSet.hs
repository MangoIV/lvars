...
instance (Show a) => Show (ISet Frzn a) where
  show (ISet lv) =
    let set = S.toList $ unsafeDupablePerformIO $ readIORef (state lv)
     in "{ISet: "
...
freezeSet (ISet (WrapLVar lv)) =
  do
    dbgChatterOnly (2) "PureSet.freezeSet: (1/2) call freezeLV"
    WrapPar $ freezeLV lv
...
insert !elm (ISet (WrapLVar lv)) = WrapPar $ putLV lv putter
 where
  putter ref = atomicModifyIORefCAS ref update
  update set =
    let set' = S.insert elm set
