...
instance LVarData1 (IMap k) where
  ...
  -- only the value.
  addHandler mh (IMap (WrapLVar lv)) callb =
    WrapPar $
      L.addHandler mh lv globalCB (\(_k, v) -> return $ Just $ unWrapPar $ callb v)
   where
    globalCB slm unlockMap =
      unWrapPar $ do
        SLM.foldlWithKey
          LI.liftIO
          (\() _k v -> forkHP mh $ callb v)
          ()
          slm
        LI.liftIO unlockMap
...
newMap mp =
  fmap (IMap . WrapLVar) $ WrapPar $ newLV $ do
    ...
    traverseWithKey_
      ( \k v -> do
          Added _ <- SLM.putIfAbsent slm k (return v)
          return ()
      )
      mp
    return slm
...
withCallbacksThenFreeze (IMap lv) callback action = do
  ...
  let deltCB (k, v) = return $ Just $ unWrapPar $ callback k v
      initCB slm unlockMap = do
        -- The implementation guarantees that all elements will be caught either here,
        -- or by the delta-callback:
        unWrapPar $ do
          SLM.foldlWithKey
            LI.liftIO
            (\() k v -> forkHP (Just hp) $ callback k v)
            ()
            slm
          LI.liftIO unlockMap -- Must happen before action.
...
forEachHP
  ...
  -- ^ callback
  -> Par e s ()
forEachHP mh (IMap (WrapLVar lv)) callb =
  WrapPar $
    L.addHandler
      mh
      lv
...
modify
  :: forall f a b e s key
   . (Ord key, Show key, Ord a, HasPut e)
  => IMap key s (f s a)
  -> key
  -- ^ The key to lookup.
  -> (Par e s (f s a))
...
gmodify
  :: forall f a b e s key
   . (Ord key, LVarData1 f, LVarWBottom f, LVContents f a, Show key, Ord a, HasPut e)
  => IMap key s (f s a)
  -> key
  -- ^ The key to lookup.
  -> (f s a -> Par e s b)
