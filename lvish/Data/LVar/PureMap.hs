...
withCallbacksThenFreeze (IMap (WrapLVar lv)) callback action =
 ...
 where
  ...
  initCB :: HandlerPool -> IV.IVar s b -> (IORef (M.Map k v)) -> IO () -> L.Par ()
  initCB hp resIV ref unlockMap = do
...
insert !key !elm (IMap (WrapLVar lv)) = WrapPar $ putLV lv putter
 where
  putter ref = atomicModifyIORef' ref update
  update mp =
    let mp' = M.insertWith fn key elm mp
        fn v1 v2
          | v1 == v2 = v1
          | otherwise = throw $ ConflictingPutExn $ "Multiple puts to one entry in an IMap!"
     in -- Here we do a constant time check to see if we actually changed anything:
...
modify (IMap lv) key newBottom fn = WrapPar $ do
  ...
  case M.lookup key mp of
    Just lv2 -> do
      L.logStrLn 3 $
        " [Map.modify] key already present: "
          ++ show key
          ++ " adding to inner "
          ++ show (unsafeName lv2)
      unWrapPar $ fn lv2
    Nothing -> do
      ...
      let putter _ = L.liftIO $ atomicModifyIORef' ref $ \mp2 ->
            case M.lookup key mp2 of
              ...
              Nothing ->
                ( M.insert key bot mp2
                ,
                  ( Just (key, bot)
                  , do
                      L.logStrLn 3 $ " [Map.modify] key absent, adding the new one."
                      unWrapPar $ fn bot
                  )
                )
...
gmodify
  :: forall f a b e s key
   . (Ord key, LVarWBottom f, LVContents f a, Show key, Ord a, HasPut e)
  => IMap key s (f s a)
  -> key
  -- ^ The key to lookup.
  -> (f s a -> Par e s b)
