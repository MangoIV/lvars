...
v3b =
  do
    chatterOTR " [v3b] Begin test: allocating sets."
    s1 <- IS.newEmptySet
    s2 <- IS.newEmptySet
    let fn e = IS.insert (e * 10) s2
...
case_i3c = do
  allowSomeExceptions ["Attempt to change a frozen LVar"] $
    do
      x <- i3c
      assertEqual
        "under-synchronized passed through"
        (S.fromList [10, 20 .. 100] :: S.Set Int)
        x
  return ()
...
case_v3d =
  assertEqual
    "test of parallelism in freezeSetAfter"
    (S.fromList [1 .. 5])
    =<< v3d
    ...
    do
      ...
      IS.freezeSetAfter s1 $ \elm -> do
        ...
        case dep of
          ...
          Just d -> do
            logDbgLn 1 $ "  [Invocation " ++ show elm ++ "] waiting on " ++ show dep
            IS.waitElem d s2
...
case_v3e =
  assertEqual
    "test of parallelism in forEachHP"
    (S.fromList [1 .. 5])
    =<< v3e
