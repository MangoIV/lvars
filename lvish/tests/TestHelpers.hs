...
assertException msgs action = do
  x <-
      ...
      (do action; return Nothing)
      ( \e -> do
          putStrLn $ "Good.  Caught exception: " ++ show (e :: SomeException)
          return (Just $ show e)
      )
  case x of
    ...
    Just s ->
      if any (`isInfixOf` s) msgs
        then return ()
          ...
          HU.assertFailure $
            "Got the wrong exception, expected one of the strings: "
              ++ show msgs
...
knownFailure act =
  do
    _ <-
      allowSomeExceptions'
        ( \_ -> do
            putStrLn "WARNING: letting through knownFailure!!!"
            return True
        )
        ""
        act
...
allowSomeExceptions' test failmsg action = do
    ...
    (do a <- action; evaluate a; return (Right a))
    ( \e ->
        do
          b <- test e
          if b
            then do
              when True $ -- (dbgLvl>=1) $
                putStrLn $
                  "Caught allowed exception: " ++ show (e :: SomeException)
              return (Left e)
            else do
              HU.assertFailure $ failmsg ++ "\nInstead got this exception:\n  " ++ show e
...
assertNoTimeOut t a = do
  ...
  case m of
    Nothing -> do
      HU.assertFailure $ "assertNoTimeOut: thread failed or timeout occurred after " ++ show t ++ " seconds"
...
timeOut interval act = do
  ...
  let loop = do
        ...
        case stat of
          ...
          ThreadDied -> do
            putStrLn " [lvish-tests] Time-out check -- thread died!"
            return Nothing
      ...
      timeCheckAndLoop = do
        ...
        if delt >= interval
          then do
            putStrLn " [lvish-tests] Time-out: out of time, killing test thread.."
            killThread tid
            -- TODO: <- should probably wait for it to show up as dead.
            return Nothing
          else do
            threadDelay (10 * 1000) -- Sleep 10ms.
            loop
  loop
