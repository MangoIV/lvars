...
class (ParMonad m) => ParFuture (m :: EffectSig -> * -> * -> *) where
  ...
  -- >    return r
  spawn
    :: (NFData a, HasPut e, FutContents m a)
    => m e s a
    -> m e s (Future m s a)
...
class (ParFuture m) => ParIVar (m :: EffectSig -> * -> * -> *) where
  ...
  put
    :: forall a e s
     . (FutContents m a, HasPut e, NFData a)
    => IVar m s a
    -> a
    -> m e s ()
...
class (ParMonad m) => LVarSched (m :: EffectSig -> * -> * -> *) where
  ...
  putLV
    ...
    -- ^ the LVar
    -> (a -> IO (Maybe d))
    -- ^ how to do the put, and whether the LVar's
    --   value changed
    -> m e s ()
