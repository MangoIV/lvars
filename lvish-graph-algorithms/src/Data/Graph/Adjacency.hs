...
nbrs AdjacencyGraph {vertOffsets, allEdges} nid =
  let ind = vertOffsets U.! (fromIntegral nid)
   ...
   in if fromIntegral nid == U.length vertOffsets - 1
        then suff
...
parReadNats
  :: forall nty e s
   . (HasGet e, HasPut e, U.Unbox nty, Num nty, Eq nty)
  => Int
  -> BS.ByteString
  -> Par e s [PartialNums nty]
      ...
      if digit hd
        then
          ( let first = U.head $ head vs -- The first (possibly partial) number parsed.
                ...
                rfrag = Just (RightFrag (fromJust $ BS.findIndex (not . digit) bs) first)
             in if total == 0
                  then case lfrg of
