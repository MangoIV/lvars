...
newtype CancelT (p :: EffectSig -> * -> * -> *) e s a = CancelT {unCancelT :: StateT CState (p e s) a}
...
data CPair = CPair
  { _stillLive :: !Bool
  , topLevel :: !Bool
  -- ^ Are NOT under a forkCancelable
  , _children :: ![CState]
  }
...
cancelMe = CancelT $ do
  ...
  if topLevel
    then error "cancelMe: cannot be used unless underneath forkCancelable"
    else error "FINISHME!  CANCEL TIME"
  ...
  live <- S.lift $
    internalLiftIO $
      atomicModifyIORef' parentRef $ \orig@(CPair bl top ls) ->
        if bl
          then -- Extend the tree by pointing to our child:
...
cancel
  :: (ParMonad p, Monad (p e s), ParMonad (CancelT p), HasPut e)
  => ThreadId
  -> CancelT p e s ()
